...
{
    /// <summary>
    /// Syncs AutoCount ARInvoice + ARInvoiceDetail to Supabase ac_invoices + ac_invoice_items.
    /// </summary>
    public static class InvoiceSyncService
    {
        public static void RunSync()
        {
            BridgeLogger.Info("=== Invoice sync starting ===");
            string jobId = null;

            var supa = SupabaseClient.FromConfig();
            if (supa == null)
            {
                BridgeLogger.Error("Invoice sync aborted: Supabase not configured.");
                return;
            }

            if (!AutoCountConnector.IsConnected)
            {
                BridgeLogger.Error("Invoice sync aborted: AutoCount not connected.");
                return;
            }

            try
            {
                jobId = SyncHelper.CreateSyncJob(supa, "invoice_sync");
                SyncHelper.UpdateJobStatus(supa, jobId, "running", null);

                DataTable invoices = AutoCountConnector.QueryTable("ARInvoice", "DocNo");
                if (invoices == null || invoices.Rows.Count == 0)
                {
                    BridgeLogger.Warn("Invoice sync: no invoices returned.");
                    SyncHelper.UpdateJobCompleted(supa, jobId, 0, 0, 0, null);
                    return;
                }

                AutoCountConnector.LogTableColumns(invoices, "ARInvoice");

                int total = invoices.Rows.Count;
                int synced = 0;
                int failed = 0;
                ...
                foreach (DataRow row in invoices.Rows)
                {
                    ...
                    try
                    {
                        string headerJson = MapInvoiceHeader(row);
                        supa.Insert("ac_invoices", headerJson, upsert: true, onConflict: "doc_no");
                    ...
                    catch (Exception ex)
                    {
                        ...
                        BridgeLogger.Warn(string.Format(
                            "Invoice sync failed for {0}: {1}", docNo, ex.Message));
                        SyncHelper.LogSyncError(supa, jobId, "invoice", docNo, ex.Message);
                    }
                }

                BridgeLogger.Info(string.Format(
                    "Invoice sync complete: {0} total, {1} synced, {2} failed.",
                    total, synced, failed));
            ...
            {
                BridgeLogger.Error("Invoice sync fatal error: " + ex.Message);
                if (jobId != null)
                    SyncHelper.UpdateJobCompleted(supa, jobId, 0, 0, 0, ex.Message);
            }

            BridgeLogger.Info("=== Invoice sync finished ===");
        }

        private static void SyncDetailLines(SupabaseClient supa, string docNo, string jobId)
        {
            DataTable details = AutoCountConnector.QueryDetailByDocNo("ARInvoiceDetail", docNo);
            if (details == null || details.Rows.Count == 0) return;

            try { supa.Delete("ac_invoice_items", "doc_no=eq." + Uri.EscapeDataString(docNo)); }
            catch (Exception ex) { BridgeLogger.Warn("Could not clear old invoice items for " + docNo + ": " + ex.Message); }
            ...
            foreach (DataRow dRow in details.Rows)
            {
                try
                {
                    string lineJson = MapInvoiceLine(dRow, docNo, seq++);
                    supa.Insert("ac_invoice_items", lineJson, upsert: false);
                }
                catch (Exception ex)
                {
                    SyncHelper.LogSyncError(supa, jobId, "invoice_item",
                        docNo + "#" + seq, ex.Message);
                }
            }
        }

        private static string MapInvoiceHeader(DataRow row)
        {
            ...
            sb.AppendFormat(",\"due_date\":{0}", SyncHelper.JsonDate(
                SyncHelper.SafeDate(row, "DueDate") ?? SyncHelper.SafeDate(row, "PaymentDueDate")));
            sb.AppendFormat(",\"amount\":{0}", SyncHelper.JsonDec(
                SyncHelper.SafeDec(row, "DocAmt") + SyncHelper.SafeDec(row, "TaxAmt")));
            sb.AppendFormat(",\"outstanding_amount\":{0}", SyncHelper.JsonDec(SyncHelper.SafeDec(row, "OutstandingAmt")));
        ...
        private static string MapInvoiceLine(DataRow row, string docNo, int seq)
        {
            var sb = new StringBuilder();
            sb.Append("{");
