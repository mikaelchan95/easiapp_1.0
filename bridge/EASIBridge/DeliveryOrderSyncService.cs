...
{
    /// <summary>
    /// Syncs AutoCount DeliveryOrder + DeliveryOrderDetail
    /// to Supabase ac_delivery_orders + ac_delivery_order_items.
    /// </summary>
    public static class DeliveryOrderSyncService
    {
        public static void RunSync()
        {
            BridgeLogger.Info("=== DeliveryOrder sync starting ===");
            string jobId = null;

            var supa = SupabaseClient.FromConfig();
            if (supa == null)
            {
                BridgeLogger.Error("DeliveryOrder sync aborted: Supabase not configured.");
                return;
            }

            if (!AutoCountConnector.IsConnected)
            {
                BridgeLogger.Error("DeliveryOrder sync aborted: AutoCount not connected.");
                return;
            }

            try
            {
                jobId = SyncHelper.CreateSyncJob(supa, "delivery_order_sync");
                SyncHelper.UpdateJobStatus(supa, jobId, "running", null);

                DataTable orders = AutoCountConnector.QueryTable("DeliveryOrder", "DocNo");
                if (orders == null || orders.Rows.Count == 0)
                {
                    BridgeLogger.Warn("DeliveryOrder sync: no delivery orders returned.");
                    SyncHelper.UpdateJobCompleted(supa, jobId, 0, 0, 0, null);
                    return;
                }

                AutoCountConnector.LogTableColumns(orders, "DeliveryOrder");
                ...
                foreach (DataRow row in orders.Rows)
                {
                    ...
                    try
                    {
                        ...
                        supa.Insert("ac_delivery_orders", headerJson, upsert: true, onConflict: "doc_no");
                    ...
                    catch (Exception ex)
                    {
                        ...
                        BridgeLogger.Warn(string.Format(
                            "DeliveryOrder sync failed for {0}: {1}", docNo, ex.Message));
                        SyncHelper.LogSyncError(supa, jobId, "delivery_order", docNo, ex.Message);
                    }
                }

                BridgeLogger.Info(string.Format(
                    "DeliveryOrder sync complete: {0} total, {1} synced, {2} failed.",
                    total, synced, failed));
            ...
            {
                BridgeLogger.Error("DeliveryOrder sync fatal error: " + ex.Message);
                if (jobId != null)
                    SyncHelper.UpdateJobCompleted(supa, jobId, 0, 0, 0, ex.Message);
            }

            BridgeLogger.Info("=== DeliveryOrder sync finished ===");
        }

        private static void SyncDetailLines(SupabaseClient supa, string docNo, string jobId)
        {
            DataTable details = AutoCountConnector.QueryDetailByDocNo("DeliveryOrderDetail", docNo);
            if (details == null || details.Rows.Count == 0) return;

            try { supa.Delete("ac_delivery_order_items", "doc_no=eq." + Uri.EscapeDataString(docNo)); }
            catch (Exception ex) { BridgeLogger.Warn("Could not clear old DO items for " + docNo + ": " + ex.Message); }
            ...
            foreach (DataRow dRow in details.Rows)
            {
                try
                {
                    ...
                    supa.Insert("ac_delivery_order_items", lineJson, upsert: false);
                }
                catch (Exception ex)
                {
                    SyncHelper.LogSyncError(supa, jobId, "delivery_order_item",
        ...
        private static string MapOrderHeader(DataRow row)
        {
            ...
            sb.AppendFormat(",\"so_doc_no\":{0}", SyncHelper.JsonStr(
                SyncHelper.FirstNonEmpty(row, "SODocNo", "SalesOrderDocNo", "FromDocNo")));
