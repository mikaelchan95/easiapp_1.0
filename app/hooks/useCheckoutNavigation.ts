...
export type CheckoutStep = 'address' | 'delivery' | 'payment' | 'review' | 'processing';
...
const CHECKOUT_STEPS: CheckoutStep[] = ['address', 'delivery', 'payment', 'review', 'processing'];
...
export function useCheckoutNavigation(initialStep: CheckoutStep = 'address') {
  ...
  const navigateToStep = useCallback((step: CheckoutStep) => {
    ...
    if (stepIndex > currentIndex) {
      ...
      const allCompleted = stepsToComplete.every(s => completedSteps.includes(s));
  ...
  const isStepAccessible = useCallback((step: CheckoutStep) => {
    const stepIndex = CHECKOUT_STEPS.indexOf(step);
    const currentIndex = CHECKOUT_STEPS.indexOf(currentStep);
