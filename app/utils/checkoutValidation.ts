 ...
 */
export function validateDeliveryAddress(address: Partial<DeliveryAddress>): AddressValidationResult {
  ...
  if (!address.postalCode?.trim()) {
    ...
    errors.push({ field: 'postalCode', message: 'Please enter a valid Singapore postal code' });
  }
  ...
  if (address.unit && address.unit.length > 50) {
    errors.push({ field: 'unit', message: 'Unit number must be less than 50 characters' });
  }

  if (address.instructions && address.instructions.length > 500) {
    errors.push({ field: 'instructions', message: 'Delivery instructions must be less than 500 characters' });
  }
 ...
 */
export function validateMinimumOrder(total: number, minimumAmount: number = 50): ValidationError | null {
 ...
 */
export function validateDeliveryTimeSlot(date: string, timeSlot: string): ValidationError | null {
  ...
  const maxDeliveryTime = new Date(currentDate.getTime() + 30 * 24 * 60 * 60 * 1000);
 ...
 */
export function validateField(field: string, value: string): { isValid: boolean; error?: string } {
  switch (field) {
    case 'name':
      if (!value?.trim()) {
    ...
    case 'postalCode':
      ...
      if (!isValidSingaporePostalCode(value)) {
        return { isValid: false, error: 'Please enter a valid Singapore postal code' };
      }
      return { isValid: true };
 ...
 */
export function validateAddress(address: any): { canContinue: boolean; errors: Record<string, string> } {
