...
export const BillingDashboard: React.FC<BillingDashboardProps> = ({
  ...
  const loadQuickStats = async () => {
    try {
      // Get invoices count
      const { data: invoices, count: invoicesCount } =
        await companyBillingService.getCompanyInvoices(companyId, { limit: 1 });

      // Get payments count
      const { data: payments, count: paymentsCount } =
        await companyBillingService.getCompanyPayments(companyId, { limit: 1 });

      // Get overdue invoices
      const { data: overdueInvoices, count: overdueCount } =
      ...
      const { data: recentPayments, count: recentCount } =
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            const now = new Date();
            const { data, error } =
              await companyBillingService.generateMonthlyInvoice(
                companyId,
                now.getMonth() + 1,
                now.getFullYear()
              );
  ...
  const renderOverviewTab = () => (
    <View style={styles.tabContent}>
      ...
      <RealTimeBalanceWidget
        ...
        onBalancePress={() =>
          navigation.navigate(
            'CompanyCreditOverview' as never,
            { companyId } as never
          )
        }
      />
        ...
        <View style={styles.statsGrid}>
          <View style={styles.statCard}>
            <Text style={styles.statValue}>
              {String(quickStats.totalInvoices)}
            </Text>
            <Text style={styles.statLabel}>Total Invoices</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statValue}>
              {String(quickStats.totalPayments)}
            </Text>
          ...
          <View style={styles.statCard}>
            <Text style={styles.statValue}>
              {String(quickStats.recentPayments)}
            </Text>
      ...
      <MonthlyOverview companyId={companyId} showHeader={false} />
    </View>
  );
                ...
                ? [
                    {
                      ...
                      onPress: () => {
                        if (alert.category === 'credit_limit') {
                          navigation.navigate(
                            'PartialPayment' as never,
                            { companyId, companyName } as never
                          );
                        }
                      },
                    },
