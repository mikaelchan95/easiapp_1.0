...
export default function CartScreen() {
  ...
  const cartItems: CartItemType[] = state.cart
    .map(item => {
      const priceWithGST = getProductPrice(
        item.product,
        state.user?.role || 'retail'
      );
  ...
  const handleAddToFavorites = (productId: string) => {
    const itemToFavorite = state.cart.find(
      item => item.product.id === productId
    );
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.summaryContent}>
          <View style={styles.summaryRow}>
            ...
            <Text style={styles.summaryValue}>
              {formatFinancialAmount(cartTotals.subtotal)}
            </Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>GST (9%)</Text>
            <Text style={styles.summaryValue}>
              {formatFinancialAmount(cartTotals.gst)}
            </Text>
          </View>
      ...
      <AnimatedFeedback
        ...
        action={
          feedback.message.includes('Undo')
            ? {
                label: 'Undo',
                onPress: handleUndo,
              }
